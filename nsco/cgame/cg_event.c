...
void CG_EntityEvent( centity_t *cent, vec3_t position ) {
	...
	case EV_FALL_SHORT:
		...
		if ((cg.DeafTime < cg.time)) trap_S_StartSound (NULL, es->number, CHAN_AUTO, cgs.media.landSound );
	...
	case EV_FALL_DEATH:
		...
		if ((cg.DeafTime < cg.time)) trap_S_StartSound (NULL, es->number, CHAN_AUTO, trap_S_RegisterSound("sound/actors/player/fall1.wav", qfalse) );
