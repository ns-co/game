...
// Navy Seals ++
	...
	{ &cg_bulletpredict, "cg_bulletpredict", "1", CVAR_ARCHIVE},
...
// defcon-x: cache digital numbers (for hud)
...
// defcon-x: cache vests
	/*
	for ( i=0 ; i<3 ; i++) {
		cgs.media.vestShaders[i] = trap_R_RegisterShaderNoMip( sb_vests[i] );
	}
	*/
// defcon-x: end
...
//	cgs.media.bloodHitShader = trap_R_RegisterShader( "bloodHit" );
	...
	if ( cgs.gametype == GT_TEAM )
		cgs.media.respawnShader = trap_R_RegisterShader( "powerups/blueflag" );
...
//	cgs.media.ammoMag_back = trap_R_RegisterShader( "gfx/2d/hud/ammo_mag.tga" );
	...
	/*
	for ( i = 0 ; i < 5 ; i++ ) {
		cgs.media.ammoMag_stripes[i] = trap_R_RegisterShader( va("gfx/2d/hud/ammo_stripes_%i", i+1) );
	} */
	for ( i = 0 ; i < 15 ; i++ ) {
...
//	cgs.media.viewBloodShader = trap_R_RegisterShader( "ns_viewBloodBlend" );
...
#if 0 // def: precache weapons when finished connecting!
	for ( i = 1 ; i < bg_numItems ; i++ ) {
		if ( items[ i ] == '1' /* || cg_buildScript.integer */  ) {
			CG_LoadingItem( i );
...
//	cgs.media.ammoMag = trap_R_RegisterModel("models/props/ammo_mag.md3");
//	cgs.media.ammoShell = trap_R_RegisterModel("models/props/ammo_shell.md3");
	...
	cgs.media.gibHeadShotBlown = trap_R_RegisterModel("models/misc/blownheads/stump.md3");
	cgs.media.gibHeadShotFace  = trap_R_RegisterModel("models/misc/blownheads/faceshot.md3");
	cgs.media.gibHeadShotNeck  = trap_R_RegisterModel("models/misc/blownheads/neckshot.md3");
...
//	cgs.media.playerSniperscap = trap_R_RegisterModel("models/players/heads/accesoires/cappy.md3");
//	cgs.media.playerheadScarp = trap_R_RegisterModel("models/players/heads/accesoires/scarf.md3");
	...
	for (i=0;i<5;i++)
	{
		...
		cgs.media.bulletholes[BHOLE_SAND][i] = trap_R_RegisterShader( va("gfx/damage/bhole_sand_%i",i+1) );
		cgs.media.bulletholes[BHOLE_SNOW][i] = trap_R_RegisterShader( va("gfx/damage/bhole_snow_%i",i+1) );
...
void CG_PrecacheWeapons( ) {
	...
	if ( ticks > cg_timetocache.integer )
		return;

	if ( !cg.snap || !cg.snap->ps.persistant )
		return;
 
	ticks++;

	if ( ticks == cg_timetocache.integer - 50 )
		CG_CenterPrint( "Precaching Weapons", SCREEN_HEIGHT * 0.30, cg_newbeeHeight.value );
	if ( ticks < cg_timetocache.integer )
		return;
  
	// only register the weapons that the server says we need
	strcpy( items, CG_ConfigString( CS_ITEMS ) ); 
