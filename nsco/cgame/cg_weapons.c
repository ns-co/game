...
==========================
*/
static void CG_40mmGrenadeTrail( centity_t *ent, const weaponInfo_t *wi ) {
	int		step;
	vec3_t	origin, lastPos;
	int		t;
	int		startTime, contents;
...
*/
	...
	if ( BG_WeaponMods( weaponNum ) & ( 1 << WM_SCOPE ) ) 
	{
		...
		weaponInfo->scopeModel = trap_R_RegisterModel( path );
		// vweap
		strcpy( path, item->world_model[0] );
		COM_StripExtension( path, path );
		strcat( path, "_scope_vweap.md3" );
		weaponInfo->v_scopeModel = trap_R_RegisterModel( path );
	...
	if ( BG_WeaponMods( weaponNum ) & ( 1 << WM_LASER ) ) 
	{
		...
		weaponInfo->lasersightModel = trap_R_RegisterModel( path );
		// vweap
		strcpy( path, item->world_model[0] );
		COM_StripExtension( path, path );
		strcat( path, "_laser_vweap.md3" );
		weaponInfo->v_lasersightModel = trap_R_RegisterModel( path ); 
	...
	if ( BG_WeaponMods( weaponNum ) & ( 1 << WM_SILENCER ) ) 
	{
		...
		weaponInfo->silencerModel = trap_R_RegisterModel( path );
		// vweap
		strcpy( path, item->world_model[0] );
		COM_StripExtension( path, path );
		strcat( path, "_silencer_vweap.md3" );
		weaponInfo->v_silencerModel = trap_R_RegisterModel( path );
	...
	if ( BG_WeaponMods( weaponNum ) & ( 1 << WM_FLASHLIGHT ) ) 
	{
		...
		weaponInfo->glModel = trap_R_RegisterModel( path );
		// vweap
		strcpy( path, item->world_model[0] );
		COM_StripExtension( path, path );
		strcat( path, "_flashlight_vweap.md3" );
		weaponInfo->v_glModel = trap_R_RegisterModel( path );
	...
	if ( BG_WeaponMods( weaponNum ) & ( 1 << WM_BAYONET ) ) 
	{
		...
		weaponInfo->bayonetModel = trap_R_RegisterModel( path );  
		// vweap
		strcpy( path, item->world_model[0] );
		COM_StripExtension( path, path );
		strcat( path, "_bayonet_vweap.md3" );
		weaponInfo->v_bayonetModel = trap_R_RegisterModel( path );
	...
	*/ 
	...
	weaponInfo->viewweaponModel  = trap_R_RegisterModel( path );
...
//		weaponInfo->firingSound = trap_S_RegisterSound( "sound/weapons/melee/fstrun.wav" );
	...
	case WP_AK47: 
		...
		weaponInfo->missileModel = trap_R_RegisterModel( "models/misc/bullets/bullet_40mm.md3" );
 		weaponInfo->missileTrailFunc = CG_40mmGrenadeTrail;
	...
	case WP_870:
		...
		// NS - 
	case WP_FLASHBANG:
		weaponInfo->missileModel = trap_R_RegisterModel( "models/weapons/flashbang/flashbang_thrown.md3" );
		...
		weaponInfo->partTags[1] = "";
  		...
  		break;
	case WP_GRENADE:
		weaponInfo->missileModel = trap_R_RegisterModel( "models/weapons/mk26/mk26_thrown.md3" );
		weaponInfo->kickBack = 0;
		...
		weaponInfo->partTags[1] = "";
 		break;
	case WP_SMOKE:
		weaponInfo->missileModel = trap_R_RegisterModel( "models/weapons/smoke/smoke_thrown.md3" );
		weaponInfo->kickBack = 0; 
	...
	while ( strlen(weaponInfo->partTags[i]) > 0 && i < MAX_WEAPONPARTS )
	{
		...
		if ( !Q_stricmp( "handle", weaponInfo->partTags[i] ) ) {
			weaponInfo->v_flashModel = trap_R_RegisterModel( va("%s_handle_vweap.md3",path) );
			strcat( path, va("_%s.md3", weaponInfo->partTags[i] )  );
		}
		else
			strcat( path, va("_1stperson_%s.md3", weaponInfo->partTags[i] )  );

		weaponInfo->weaponParts[i] = trap_R_RegisterModel( path );
...
#define FLASHLIGHT_RANGE	2000
		...
		//trap_R_AddAdditiveLightToScene( trace.endpos, 200, 0.3f,0.3f,0.3f);
		//
		// BLUTENGEL 20040207
		/*trap_R_AddLightToScene( trace.endpos, 150, 1.0f-trace.fraction,1.0f-trace.fraction,1.0f-trace.fraction);
		trap_R_AddLightToScene( trace.endpos, 200, 1.0f-trace.fraction,1.0f-trace.fraction,1.0f-trace.fraction);*/
		VectorMA(trace.endpos, -32.0, forward, trace.endpos);
		trap_R_AddAdditiveLightToScene( trace.endpos, 300-trace.fraction*150, 0.02f, 0.02f, 0.03f);
		trap_R_AddAdditiveLightToScene( trace.endpos, 100-trace.fraction*50, 0.08f, 0.08f, 0.10f);
		...
		{ 
			if ( pointinfrontmindot( cent->currentState.apos.trBase,endPoint, cg.refdef.vieworg , -0.3 + test_x.value ) )
			{
			...
			for ( s = 0;  s <  cg.Inventorypos[cg.activeInventory-1]; s++ )
			{
				...
				{		
					...
					if ( cg.Inventory[ cg.activeInventory -1][s] <= 0 ) {
						...
						// CG_RegisterWeapon(  cg.Inventory[ cg.activeInventory-1 ] [ s ] );
...
// firing using a silenced weapon , play a silenced sound then  
	if ( othermode )
	{
		...
		if ( c > 0 ) {
			...
			if ( weap->otherflashSound[c] )
			{
				if ((cg.DeafTime < cg.time)) trap_S_StartSound( NULL, ent->number, CHAN_WEAPON, weap->otherflashSound[c] );
			}
		}
	}
	else if ( cent->currentState.eFlags & EF_SILENCED )
	{
		...
		if ( c > 0 ) {
			...
			if ( weap->sil_flashSound[c] )
			{
				if ((cg.DeafTime < cg.time)) trap_S_StartSound( NULL, ent->number, CHAN_WEAPON, weap->sil_flashSound[c] );
			}
		}
	}
	else {
		...
		if ( c > 0 ) {
			...
			if ( weap->flashSound[c] )
			{
				if ((cg.DeafTime < cg.time)) trap_S_StartSound( NULL, ent->number, CHAN_WEAPON, weap->flashSound[c] );
			}
		}
	}
...
*/  
	...
	{
		if (cg_bulletpredict.integer) CG_PredictFireShotgun( );
	}
...
void CG_MissileHitWall( int weapon, int clientNum, vec3_t origin, vec3_t dir, int soundtype, int surfaceparms) {
	...
	else if ( soundtype == 2 )
	{
		...
		if ((cg.DeafTime < cg.time)) trap_S_StartSound( origin, ENTITYNUM_WORLD, CHAN_AUTO, cgs.media.bulletHitHelmet[rndnum] );
	}
	else if ( metal ) 
	{
 		char sound[64];
		...
		Com_sprintf(sound,sizeof(sound),"sound/misc/metal_ric%i.wav", rndnum);

		if ((cg.DeafTime < cg.time)) trap_S_StartSound ( origin , ENTITYNUM_WORLD, CHAN_AUTO, trap_S_RegisterSound( sound, qfalse )  );
	}
	else if ( sfx ) {
...
void CG_WeaponAnimation( playerState_t *ps ) {
 ...
 // REMOVING AS DEMOCRITUS REQUESTED for
 // modelling the new weapon positions
/*#ifdef SAME_WEAPONPIPE
	weaponMod[0] = -4;
	weaponMod[1] = -3;
#endif */
/*	if ( cg_smallGuns.integer || cg_drawGun.integer == 3 || cg_drawGun.integer == 4 )
	{
		...
		VectorMA( hand.origin, weaponMod[0], cg.refdef.viewaxis[0], hand.origin);
		VectorMA( hand.origin, weaponMod[1], cg.refdef.viewaxis[1], hand.origin);
		VectorMA( hand.origin, weaponMod[2], cg.refdef.viewaxis[2], hand.origin);
	}*/
