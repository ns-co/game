...
{ 
	...
	else
	{
		...
		CG_Text_Paint( x,y+13, 0.25f, colorWhite, text, 0,0,ITEM_TEXTSTYLE_OUTLINED);
	}
}
...
//					CG_Printf("Parsed (S line %i) : %s\n", sealBriefingLines, SealBriefing[ sealBriefingLines ] );
					...
					if ( strlen ( SealBriefing[ sealBriefingLines ] ) + strlen ( token ) > 44 )
						sealBriefingLines++;
...
//					CG_Printf("Parsed (T line %i) : %s\n", tangoBriefingLines, TangoBriefing[ tangoBriefingLines ] );
					...
					// create automated linebreak
					if ( strlen ( TangoBriefing[ tangoBriefingLines ] ) + strlen ( token ) > 44 )
						tangoBriefingLines++;
...
void CG_DrawInformation( void ) {
	...
	if ( !levelshot ) {
		levelshot = trap_R_RegisterShaderNoMip( "gfx/2d/unknownmap" );
	}
	...
	CG_DrawLoadingBar( 16, 448,608,16 );
	...
	y = INFO_STRING_START + INFO_STRING_DIST*3;
	...
	if ( !atoi( buf ) || 1) {
		...
		Q_CleanStr(buf);
		CG_Text_Paint( 300,y, 0.2f, colorWhite, buf, 0,0,0);
		...
		if ( s[0] == '1' ) {
			CG_Text_Paint( 300,y, 0.2f, colorWhite, "Pure Server", 0,0,0);
		...
		if ( s[0] ) {
			CG_Text_Paint( 300,y, 0.2f, colorWhite,s, 0,0,0);
	...
	if ( s[0] == '1' ) {
		CG_Text_Paint( 300,y, 0.2f, colorWhite, va("Cheats are enabled" ), 0,0,0);
		y += INFO_STRING_DIST;
	...
	CG_Text_Paint( 300,y, 0.2f, colorWhite,  va("Gametype is %s", s ), 0,0,0);

	y += INFO_STRING_DIST;
		
	value = atoi( Info_ValueForKey( info, "timelimit" ) );
	if ( value ) {
		CG_Text_Paint( 300,y, 0.2f, colorWhite,  va("Timelimit is set to: %i", value ), 0,0,0);
		y += INFO_STRING_DIST;
	}
	...
	if (cgs.gametype < GT_TEAM ) {
		...
		if ( value ) {
			CG_Text_Paint( 300,y, 0.2f, colorWhite,  va("Fraglimit is set to: %i", value ), 0,0,0);
	...
	if (cgs.gametype >= GT_TEAM) {
		...
		if ( value ) {
			CG_Text_Paint( 300,y, 0.2f, colorWhite,  va("Pointlimit is set to: %i", value ), 0,0,0);
	...
	if ( sealBriefingLines > 0 ) {
		...
		{
			CG_Text_Paint( 16,y, 0.25f, colorWhite, SealBriefing[value], 0,0,ITEM_TEXTSTYLE_SHADOWED);
	...
	if ( tangoBriefingLines > 0 ) {
		...
		{
			CG_Text_Paint( 336,y, textscale, colorWhite, TangoBriefing[value], 0,0,ITEM_TEXTSTYLE_SHADOWED);
			y += CG_Text_Height( TangoBriefing[value], textscale, 0 ) + 3;
		}	
 	}

	s = "Navy Seals : Covert Operations - Beta 1.93";
	y = CG_Text_Height( s, 0.2f, 0 );

	CG_Text_Paint( 4, y+2, 0.2f,colorWhite, s,0,0,4 );
