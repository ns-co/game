...
/*
===============
CG_Draw3rdPersonPlayerCrosshair

Draw a crosshair at the end of playersview
===============
*/
static void CG_Draw3rdPersonPlayerCrosshair( centity_t *cent ) {
	vec3_t		start, end;
	trace_t		trace;
//	int			contents;
//	polyVert_t	verts[4];
	...
	endDist = Distance( start, trace.endpos );

	ex = CG_AllocLocalEntity();
	ex->leType = LE_SCALE_FADE;

	ex->startTime = cg.time;
	ex->endTime = ex->startTime + 1;	

	VectorCopy ( trace.endpos, ex->refEntity.origin);
	ex->refEntity.reType = RT_SPRITE;
	ex->refEntity.rotation = sin( cg.time );
	ex->refEntity.radius = 24;
	VectorMA( ex->refEntity.origin, -1, forward, ex->refEntity.origin );
	...
	ex->refEntity.customShader = cgs.media.crosshairShader[ ca % NUM_CROSSHAIRS ];
 
 
	ex2 = CG_AllocLocalEntity();
	ex2->leType = LE_SCALE_FADE;

	ex2->startTime = cg.time;
	ex2->endTime = ex2->startTime + 1;	

	VectorCopy ( trace.endpos, ex2->refEntity.origin);
	VectorMA( ex2->refEntity.origin, -endDist/3, forward, ex2->refEntity.origin );
	ex2->refEntity.reType = RT_SPRITE;
	ex2->refEntity.rotation = sin( cg.time );
	ex2->refEntity.radius = 24; 
	...
	ex2->refEntity.customShader = cgs.media.crosshairShader[ ca % NUM_CROSSHAIRS ];
 
	ex3 = CG_AllocLocalEntity();
	ex3->leType = LE_SCALE_FADE;

	ex3->startTime = cg.time;
	ex3->endTime = ex3->startTime + 1;	

	VectorCopy ( trace.endpos, ex3->refEntity.origin);
	VectorMA( ex3->refEntity.origin, (-endDist/3)*2, forward, ex3->refEntity.origin );
	ex3->refEntity.reType = RT_SPRITE;
	ex3->refEntity.rotation = sin( cg.time );
	ex3->refEntity.radius = 24; 
	...
	ex3->refEntity.customShader = cgs.media.crosshairShader[ ca % NUM_CROSSHAIRS ];
...
static qboolean CG_RegisterClientModelname( clientInfo_t *ci, const char *modelName, const char *headName ) {
	...
	ci->leftArmModel = trap_R_RegisterModel( filename );
	if ( !ci->leftArmModel ) {
		if ( cg_buildScript.integer )
	...
	ci->rightArmModel = trap_R_RegisterModel( filename );
	if ( !ci->rightArmModel ) {
		if ( cg_buildScript.integer )
...
void CG_AddRefEntityWithPowerups( refEntity_t *ent, entityState_t *state, int team ) {
	...
	if ( state->eFlags & EF_REDGLOW && cgs.clientinfo[ state->clientNum ].team == cg.snap->ps.persistant[PERS_TEAM] )
	{
		ent->customShader = cgs.media.respawnShader;		 
		trap_R_AddRefEntityToScene( ent );
	}
