...
#endif
#if 0 // this is the old function, prvoided by idsoftware
void BotChooseWeapon(bot_state_t *bs) {
	int newweaponnum;
...
void BotUpdateInventory(bot_state_t *bs) {
	...
	//armor
//	bs->inventory[INVENTORY_ARMOR] = (bs->cur_ps.powerups & ( 1 << PW_VEST) );
	//weapons
	/*
	bs->inventory[INVENTORY_GAUNTLET] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_KNIFE)) != 0;
...
qboolean BotIsObserver(bot_state_t *bs) {
	char buf[MAX_INFO_STRING];
	if (bs->cur_ps.pm_type == PM_SPECTATOR || bs->cur_ps.pm_type == PM_NOCLIP) return qtrue;
...
bot_moveresult_t BotAttackMove(bot_state_t *bs, int tfl) {
	...
	if ( BG_IsMelee ( bs->cur_ps.weapon ) ) {
		attack_dist = 0;
		attack_range = 0;
	}
	else {
...
//	bot_goal_t goal;
	...
	//
	if (BG_IsRifle(wi.number) || BG_IsPistol(wi.number) || BG_IsSMG(wi.number) ||
		BG_IsShotgun(wi.number ) ) {
		//distance towards the enemy
		dist = VectorLength(dir);
...
int BotGetActivateGoal(bot_state_t *bs, int entitynum, bot_activategoal_t *activategoal) {
	...
	//if it is a door 
 	if (!strcmp(classname, "func_explosive") ||
		!strcmp(classname, "func_explosive_wood") ||
		!strcmp(classname, "func_explosive_metal") ||
		!strcmp(classname, "func_explosive_stone") ||
		!strcmp(classname, "func_explosive_glass") ) 
	{ 
...
void BotCheckForGrenades(bot_state_t *bs, entityState_t *state) {
	...
	if (state->eType != ET_MISSILE || ( state->weapon != WP_GRENADE && state->weapon  != WP_FLASHBANG) )
		return;
...
void BotSetupDeathmatchAI(void) {
	...
	//
/*	if (gametype == GT_CTF) {
		if (trap_BotGetLevelItemGoal(-1, "Briefcase Returnpoint", &ctf_blueflag) < 0)
			BotAI_Print(PRT_WARNING, "CTF without Red Flag\n");
		if (trap_BotGetLevelItemGoal(-1, "Briefcase", &ctf_redflag) < 0)
