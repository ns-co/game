...
{   
	...
	// splash damage
	if ( ent->splashDamage ) {
		G_RadiusDamage( ent->s.origin, ent, ent->splashDamage, ent->splashRadius, NULL, ent->splashMethodOfDeath );
	} 
//	G_Printf("explosion_explode\n");
}
...
void model_die ( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath ) {
	...
	if ( !Q_stricmp( inflictor->classname , "reallead") )
	{
		VectorCopy( inflictor->movedir, dir );
	}
	else if ( BG_IsGrenade( inflictor->s.weapon ) )
	{
