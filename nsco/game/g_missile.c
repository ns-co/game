...
#define	MISSILE_PRESTEP_TIME	100
...
//		G_Printf("grenadepoint stuck. changing: %f\n", origin[2] );
	...
	for ( i = 0 ; i < level.maxclients ; i++ ) 
	{
		...
		trap_Trace( &tr, org, NULL, NULL, origin, client->ps.clientNum, ( CONTENTS_SOLID | CONTENTS_BODY | CONTENTS_LAVA | CONTENTS_SLIME ) );
...
void G_RunMissile( gentity_t *ent ) {
	...
	/*
	if (inwater && !(!Q_stricmp(ent->classname , "reallead"))  )
		VectorClear(ent->s.pos.trDelta ); // remove vector 
	*/ 

	// this is bullshit, only for nades to stay in air or have no bounding box
	// sux ...
	//if ( BG_IsGrenade( ent->s.weapon ) 
	//	&& ( ent->timestamp + 500 < level.time ) 
