...
static int PM_FootstepForSurface( void ) {
	...
	//	CG_Printf("Walking on dirt!\n");
		return EV_FOOTSTEP_DIRT;
	}
	if ( pml.groundTrace.surfaceFlags & SURF_SNOWSTEPS ) {

		//CG_Printf("Walking on snow!\n");
		return EV_FOOTSTEP_SNOW;
	}
	if ( pml.groundTrace.surfaceFlags & SURF_WOODSTEPS ) {
	//	CG_Printf("Walking on wood!\n");
		return EV_FOOTSTEP_WOOD;
	}

	// Navy Seals ++
	//  fix me!         : FOOTSTEPS FOR SURFACE !! 
	/*
	
 	new NSQ3 surfaceparms 


Abgesehen von den sounds unterscheiden sich die Surfaceparm materialien auch noch in den bullet-hit-wall effekten.



(default)		- Für stein sowie plastik und anderes unspektakuläres material.
				minimales rausbröseln und standard q3 auftreff effekt	
			
surfaceparm metalsteps	- Metal. Ist schon in Q3 drin. schwer durchdringbar.
				fette funken und standard q3 auftreff effekt
				
surfaceparm glass	- Glass. Leicht durchdringbar
				kein auftreff effekt, nur minimale Glassplitter fliegen
				& bröseln herum.

surfaceparm wood	- Holz. Leicht durchdringbar
				Wie glass, nur mit Holz splittern und anderem sound. Dazu
				noch etwas gebrösel

surfaceparm soft	- Für grass, Sand und anders. Am leichtesten durchdringbar
				Standard q3 auftreff effekt und statt rausbröseln wie 
				bei standard spritzt hier das selbe zeug hoch.


surfaceparm steps	- zusätzlich zu anderen surfaceparms. 
			  	immer fußspuren auf dem material hinterlassen (z.B. Schnee)

surfaceparm nosteps	- zusätzlich zu anderen surfaceparms. 
			  	Niemals fußspuren auf dem material hinterlassen (z.B. Clip
				Texes). Z.B wenn aus wasser rausgekommen


	standard q3 auftreff effekt (model mit animierter map das bei bullet-hit-wall
			gespwant wird)

	rausbröseln (kleine punkte in verschiedenen Graustufen (mittlere werte bis schwarz)
			bröseln wie in Half Life oder Q2 particle like nach unten raus
			oder spritzen nach oben raus)
	
	funken (wie bereits reingecodet, aber mit etwas besserem shader, so das sie etwas
			fetter heraus kommen)

	glas/holz splitter (minimale 1 polygon objekte) fliegen auf der anderen seite 
			sehr schnell heraus und fallen etwas langsamer auf der eintritts-
			seite herunter. so 5-10 bröckelchen. 



	footsteps (werden auf "surfaceparm steps" materalien hinterlassen und auch kurze 
			zeit noch wenn man aus dem wasser kommt. Verblassen langsam)
	*/
	// Navy Seals --
	return EV_FOOTSTEP;
}
