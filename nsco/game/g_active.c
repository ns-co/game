...
void G_SetClientSound( gentity_t *ent ) {
		...
		ent->client->ps.pm_type != PM_NORMAL )
	{
 	...
 	if (ent->waterlevel && (ent->watertype & ( CONTENTS_LAVA|CONTENTS_SLIME ) ) ) {
		...
		(
		//(
			//( ent->client->buttons & BUTTON_SPRINT && ent->client->ps.stats[STAT_STAMINA] < 200 ) ||
			( /* ent->client->ps.stats[STAT_STAMINA] < 30 ||*/ ent->health < 10 ) 
		//) && ent->health > 0 &&
		//( NS_GotPowerup( ent, PW_VEST ) || NS_GotPowerup( ent, PW_HELMET ) )
		)
	{
