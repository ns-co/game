...
void NS_DropWeapon ( gentity_t *ent)
{
	...
	// roundstarttime to see if we can drop weapons already
//	roundStartTime = level.roundstartTime; 
	// remove roundtime to get time the round started.
	
//  	if ( ( /* level.vip[TEAM_RED] == VIP_ESCAPE || */ level.vip[TEAM_BLUE] == VIP_STAYALIVE || level.vip[TEAM_RED] == VIP_STAYALIVE ) && !g_overrideGoals.integer)
