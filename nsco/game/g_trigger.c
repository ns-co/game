...
void multi_trigger( gentity_t *ent, gentity_t *activator ) {
	...
	if ( ent->wait > 0 ) {
		...
		ent->think = trap_UnlinkEntity;
		// defcon don't free, unlink so we can relink it later. 
	}
}
...
Triggers only if the client is pressing the use button
...
*/
void Touch_Trigger_Use(gentity_t *ent, gentity_t *other, trace_t *trace ) {
	trace_t tr;
...
void SP_trigger_use( gentity_t *ent ) {
	...
	ent->touch = Touch_Trigger_Use;

	ent->r.contents = CONTENTS_TRIGGER;

	ent->classname = "trigger_use";
...
void Use_Toggle( gentity_t *ent, gentity_t *other, gentity_t *activator ) {
	if (ent->count > 0) ent->count = 0;
	else ent->count = 1;
}

void Touch_Toggle( gentity_t *self, gentity_t *other, trace_t *trace ) {
	if( (!other->client) || (self->count > 0)) {
		return;
	}

	if (self->ns_team != 2 && !Is_OnBrushTeam(self,other) )
		return;
	
	multi_trigger( self, other );
} 

/*QUAKED trigger_toggle (.5 .5 .5) TOGGLE

Press toggle to deactivate the trigger on start.
...
void SP_trigger_toggle( gentity_t *ent ) {
	...
	if ( ent->random >= ent->wait && ent->wait >= 0 ) {
		ent->random = ent->wait - FRAMETIME;
		G_Printf( "trigger_toggle has random >= wait\n" );
	}

	ent->count = ent->spawnflags;

	ent->touch = Touch_Toggle;
	ent->use = Use_Toggle;
...
void TriggerToggle_ResetState( gentity_t *ent ) {
	ent->count = ent->spawnflags;
}
