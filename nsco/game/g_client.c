...
void ClientUserinfoChanged( int clientNum ) {
	...
	if (g_gametype.integer >= GT_TEAM && g_entities[clientNum].r.svFlags & SVF_BOT) {
		...
		if ( !Q_stricmp( s, "seals" ) || !Q_stricmp( s, "s" ) ) {
			team = TEAM_RED;
		} else if ( !Q_stricmp( s, "tangos" ) || !Q_stricmp( s, "t" ) ) {
			team = TEAM_BLUE;
		} else {
		...
		// bot class shit 
			float a = random() ;
			int class_num = CLASS_NONE;

			if (a <= 0.2)
				class_num = CLASS_RECON;
			else if (a <= 0.4)
				class_num = CLASS_ASSAULT;
			else if (a <= 0.6)
				class_num = CLASS_HEAVYSUPPORT;
			else if ( a <= 0.8 )
				class_num = CLASS_SNIPER; 
			else
				class_num = CLASS_DEMOMAN; 

			SetClass(ent, class_num); 
		}  
	}
	else {
