...
int BotAI(int client, float thinktime) {
	...
	{
		int i,primary,secondary,melee,grenade; 
		gitem_t *item;

		primary=secondary=melee=grenade = WP_NONE;

		for ( i = WP_NUM_WEAPONS - 1 ; i > 0 ; i-- ) {
			if ( BG_GotWeapon( i, bs->cur_ps.stats ) ) {
				// get some stuff for weapon selection
				if ( BG_IsPrimary(i) )
				{
					primary = i;
					item = BG_FindItemForWeapon( i );

					if ( bs->cur_ps.ammo[item->giAmmoTag] <= 0 )
						primary = WP_NONE;
				}
		...
		if ( primary )
			bs->weaponnum = primary;
		else if ( secondary )
			bs->weaponnum = secondary;
		else if ( grenade )
			bs->weaponnum = i;
		else if ( melee )
			bs->weaponnum = i;
