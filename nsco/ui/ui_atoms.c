...
qboolean UI_ConsoleCommand( int realTime ) {
	...
	// navy seals. open radio menu. open player menu
	if ( Q_stricmp (cmd, "ui_radiomenu") == 0 ) {
		Menus_OpenByName("ingame_radio");
		// open radio menu
		return qtrue;
	}
	if ( Q_stricmp (cmd, "ui_playermenu") == 0 ) {
		// open playermenu only if ui_team > 0
		int team = trap_Cvar_VariableValue( "ui_team" );

		if ( team == 1 || team == 2 ) // red
		{
			Menus_OpenByName("ingame_char");
		}
		else
			Com_Printf("You must be in a team to open player menu.\n");
