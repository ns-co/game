...
// i tried to remove the GT_* defines but it always crashed.
#define GT_OBELISK	10
#define GT_HARVESTER 11
#define GT_TOURNAMENT	12
...
void _UI_Refresh( int realtime )
{
	...
	xpo = trap_Cvar_VariableValue("char_xp");
	if (xp < 0) { // first time on server
		if (xpo != 0) {
...
static void UI_DrawOpponent(rectDef_t *rect) {
  ...
  char team[256];
    ...
    UI_PlayerInfo_SetInfo( &info2, LEGS_IDLE, TORSO_STAND_RIFLE, viewangles, vec3_origin, WP_M4, qfalse );
...
static void UI_OwnerDraw(float x, float y, float w, float h, float text_x, float text_y, int ownerDraw, int ownerDrawFlags, int align, float special, float scale, vec4_t color, qhandle_t shader, int textStyle) {
  ...
  rect.h = h; 
	...
	case UI_SEAL_PLAYERMODEL:
		UI_DrawPlayerModelForTeam( &rect, TEAM_BLUE );
		break;
	case UI_TANGO_PLAYERMODEL:
		UI_DrawPlayerModelForTeam( &rect, TEAM_RED );
		break;
    case UI_CLANNAME:
...
void CG_FreeXP( void ) {
	...
	for (i = accuracy; i>1; i--) xp += i;
	for (i = strength; i>1; i--) xp += i;
	for (i = stamina; i>1; i--) xp += i;
	for (i = stealth; i>1; i--) xp += i;
	for (i = technical; i>1; i--) xp += i;
	for (i = speed; i>1; i--) xp += i;
...
static qboolean CG_HandleGrenadeButton( int flags, float *special, int key ) {
	...
	if (key != K_MOUSE1 && key != K_MOUSE2 ) return qfalse;
	...
	switch ( flags ) {
		...
		case CG_INVEN_AMMO_FLASH:
			if ( key == K_MOUSE1 ) flash++; else flash--; 
			
			if (flash < 0) flash = 0; if (flash > 2) flash = 2;
			
			if (flash + smoke + mk26 + gren40mm > SEALS_MAX_GRENADES ) flash--;
		...
		case CG_INVEN_AMMO_SMOKE:
			if ( key == K_MOUSE1 ) smoke++; else smoke--; 
			
			if (smoke < 0) smoke = 0; if (smoke > 2) smoke = 2;
			
			if (flash + smoke + mk26 + gren40mm > SEALS_MAX_GRENADES ) smoke--;
		...
		case CG_INVEN_AMMO_MK26:
			if ( key == K_MOUSE1 ) mk26++; else mk26--; 
			
			if (mk26 < 0) mk26 = 0; if (mk26 > 2) mk26 = 2;
			
			if (flash + smoke + mk26 + gren40mm > SEALS_MAX_GRENADES ) mk26--;
		...
		case CG_INVEN_AMMO_40MM:
			if ( key == K_MOUSE1 ) gren40mm++; else gren40mm--; 
			
			if (gren40mm < 0) gren40mm = 0; if (gren40mm > 2) gren40mm = 2;
			
			if (flash + smoke + mk26 + gren40mm > SEALS_MAX_GRENADES ) gren40mm--;
...
static void UI_BuildQ3Model_List( void )
{
	...
	for (i=0; i<numdirs && uiInfo.q3HeadCount < MAX_PLAYERMODELS; i++,dirptr+=dirlen+1)
	{
		...
		for (j=0; j<numfiles && uiInfo.q3HeadCount < MAX_PLAYERMODELS;j++,fileptr+=filelen+1)
		{
			...
			// look for icon_????
			if ( Q_stricmpn(skinname, "preview_", 8) == 0 )
			{
				if (Q_stricmp(skinname, "preview_") == 0) {
...
void UI_RegisterCvars( void ) {
	...
	// BLUTENGEL 07.01.2004
	// removed as noone really wants to know that!
	// Com_Printf("UI: hunkmegs: %i zonemegs: %i soundmegs: %i\n", hunkmegs,zonemegs,soundmegs );
	...
	if ( hunkmegs < 135 )
	{
		trap_Error( va( 
			...
			"Your 'com_hunkmegs' setting is %i, it must be atleast 135."
	...
	if ( soundmegs < 8 )
	{
		trap_Error( va( 
			...
			"You might have started NS-CO via the Quake3 mods menu."
			"Please use a shortcut, start with the required parameters or use the launcher."
			"Your 'com_soundmegs' setting is %i, it must be atleast 8."
