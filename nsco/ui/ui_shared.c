...
qboolean ItemParse_ownerParam( itemDef_t *item, int handle );
...
void GradientBar_Paint(rectDef_t *rect, vec4_t color) {
	...
	DC->setColor( color );
	if ( rect->novscreen )
		DC->drawStretchPic(rect->x, rect->y, rect->w, rect->h, 0, 0, 1, 1, DC->Assets.gradientBar );
	else
...
void Window_Paint(Window *w, float fadeAmount, float fadeClamp, float fadeCycle) {
  ...
  if (w->style == WINDOW_STYLE_FILLED) {
	  	...
	  	if ( w->flags & WINDOW_TEAMCOLOR )
		{
	#ifdef CGAME
			if ( cg.snap->ps.persistant[PERS_TEAM] == TEAM_RED )
			{
				bcolor[0] = DC->Assets.team1color[0];
				bcolor[1] = DC->Assets.team1color[1];
				bcolor[2] = DC->Assets.team1color[2];
				bcolor[3] = DC->Assets.team1color[3];
			}
			else if ( cg.snap->ps.persistant[PERS_TEAM] == TEAM_BLUE )
			{
				bcolor[0] = DC->Assets.team2color[0];
				bcolor[1] = DC->Assets.team2color[1];
				bcolor[2] = DC->Assets.team2color[2];
				bcolor[3] = DC->Assets.team2color[3];
			}
			else
			{
				bcolor[0] = DC->Assets.dmcolor[0];
				bcolor[1] = DC->Assets.dmcolor[1];
				bcolor[2] = DC->Assets.dmcolor[2];
				bcolor[3] = DC->Assets.dmcolor[3];
			}
	#endif
		}
		else
		{
			bcolor[0] = w->backColor[0];
			bcolor[1] = w->backColor[1];
			bcolor[2] = w->backColor[2];
			bcolor[3] = w->backColor[3];
		}

    // box, but possible a shader that needs filled
		if (w->background) {
			Fade(&w->flags, &bcolor[3], fadeClamp, &w->nextTime, fadeCycle, qtrue, fadeAmount);
			DC->setColor(bcolor);
			if ( fillRect.novscreen )
				DC->drawStretchPic(fillRect.x, fillRect.y, fillRect.w, fillRect.h, 0, 0, 1, 1, w->background );
			else
...
#ifdef CGAME
		if ( cg.snap->ps.persistant[PERS_TEAM] == TEAM_RED )
			GradientBar_Paint(&fillRect, DC->Assets.team1color );
		else if ( cg.snap->ps.persistant[PERS_TEAM] == TEAM_BLUE )
			GradientBar_Paint(&fillRect, DC->Assets.team2color );
		else
			GradientBar_Paint(&fillRect, DC->Assets.dmcolor );
#endif
	}
    else 
...
#ifdef CGAME
		if ( cg.snap->ps.persistant[PERS_TEAM] == TEAM_RED )
			DC->setColor(DC->Assets.team1color );
		else if ( cg.snap->ps.persistant[PERS_TEAM] == TEAM_BLUE )
			DC->setColor(DC->Assets.team2color );
		else
			DC->setColor(DC->Assets.dmcolor );
#endif
	}
  ...
  if (w->border == WINDOW_BORDER_FULL) {
	...
	if ( w->flags & WINDOW_TEAMCOLOR )
	{
		...
		if ( cg.snap->ps.persistant[PERS_TEAM] == TEAM_RED )
		{
			color[0] = DC->Assets.team1color[0];
			color[1] = DC->Assets.team1color[1];
			color[2] = DC->Assets.team1color[2];
			color[3] = DC->Assets.team1color[3];
		}
		else if ( cg.snap->ps.persistant[PERS_TEAM] == TEAM_BLUE )
		{
			color[0] = DC->Assets.team2color[0];
			color[1] = DC->Assets.team2color[1];
			color[2] = DC->Assets.team2color[2];
			color[3] = DC->Assets.team2color[3]; 
		}
		else
		{
			color[0] = DC->Assets.dmcolor[0];
			color[1] = DC->Assets.dmcolor[1];
			color[2] = DC->Assets.dmcolor[2];
			color[3] = DC->Assets.dmcolor[3]; 
		} 
...
Write a configfile for the player
=================
*/
void UI_WriteWeaponConfig( const char *filename ) {
	fileHandle_t	configFile; 
	char			string[1024]; 
	char			var[MAX_TOKEN_CHARS];

	if ( !Q_stricmp(filename,"autoteam") )
	{
		int team;

		trap_Cvar_VariableStringBuffer("ui_team", var , sizeof(var ) );

		team = atoi(var);

		if ( team == 1 ) // red
			Com_sprintf( string, sizeof(string), "configs/seals.cfg" );
		else if ( team == 2 )
			Com_sprintf( string, sizeof(string), "configs/tangos.cfg" );
		else
			Com_sprintf( string, sizeof(string), "configs/ffa.cfg" );
	}
	else
		Com_sprintf( string, sizeof(string), "configs/%s", filename );
	
	trap_FS_FOpenFile( string, &configFile, FS_WRITE );

	if ( !configFile ) {
		return;
	} 

	Com_sprintf( string, sizeof(string), "// This file has been generated by Navy Seals\n// Navy Seals:Covert Operations (c) 2000-2001 team-mirage\n// http://www.team-mirage.net\n" );
	trap_FS_Write( string, strlen( string ), configFile );

	//
	// secondaries
	//
	trap_Cvar_VariableStringBuffer("inven_lasersight_secondary", var , sizeof(var ) );
	Com_sprintf( string, sizeof(string), "seta inven_lasersight_secondary %i\n", atoi(var) );
	trap_FS_Write( string, strlen( string ), configFile );

	trap_Cvar_VariableStringBuffer("inven_bayonet_secondary", var , sizeof(var ) );
	Com_sprintf( string, sizeof(string), "seta inven_bayonet_secondary %i \n", atoi(var) );
	trap_FS_Write( string, strlen( string ), configFile );

	trap_Cvar_VariableStringBuffer("inven_grenadelauncher_secondary", var , sizeof(var ) );
	Com_sprintf( string, sizeof(string), "seta inven_grenadelauncher_secondary %i \n", atoi(var) );
	trap_FS_Write( string, strlen( string ), configFile );

	trap_Cvar_VariableStringBuffer("inven_scope_secondary", var , sizeof(var ) );
	Com_sprintf( string, sizeof(string), "seta inven_scope_secondary %i \n", atoi(var) );
	trap_FS_Write( string, strlen( string ), configFile );

	trap_Cvar_VariableStringBuffer("inven_silencer_secondary", var , sizeof(var ) );
	Com_sprintf( string, sizeof(string), "seta inven_silencer_secondary %i \n", atoi(var) );
	trap_FS_Write( string, strlen( string ), configFile ); 
	...
	Com_sprintf( string, sizeof(string), "seta inven_lasersight %i\n", atoi(var) );
	trap_FS_Write( string, strlen( string ), configFile );

	trap_Cvar_VariableStringBuffer("inven_bayonet", var , sizeof(var ) );
	Com_sprintf( string, sizeof(string), "seta inven_bayonet %i \n", atoi(var) );
	trap_FS_Write( string, strlen( string ), configFile );

	trap_Cvar_VariableStringBuffer("inven_grenadelauncher", var , sizeof(var ) );
	Com_sprintf( string, sizeof(string), "seta inven_grenadelauncher %i \n", atoi(var) );
	trap_FS_Write( string, strlen( string ), configFile );

	trap_Cvar_VariableStringBuffer("inven_scope", var , sizeof(var ) );
	Com_sprintf( string, sizeof(string), "seta inven_scope %i \n", atoi(var) );
	trap_FS_Write( string, strlen( string ), configFile );

	trap_Cvar_VariableStringBuffer("inven_duckbill", var , sizeof(var ) );
	Com_sprintf( string, sizeof(string), "seta inven_duckbill %i \n", atoi(var) );
	trap_FS_Write( string, strlen( string ), configFile );

	trap_Cvar_VariableStringBuffer("inven_flashlight", var , sizeof(var ) );
	Com_sprintf( string, sizeof(string), "seta inven_flashlight %i \n", atoi(var) );
	trap_FS_Write( string, strlen( string ), configFile );

	trap_Cvar_VariableStringBuffer("inven_silencer", var , sizeof(var ) );
	Com_sprintf( string, sizeof(string), "seta inven_silencer %i \n", atoi(var) );
	trap_FS_Write( string, strlen( string ), configFile ); 

	// items

	trap_Cvar_VariableStringBuffer("inven_helmet", var , sizeof(var ) );
	Com_sprintf( string, sizeof(string), "seta inven_helmet %i \n", atoi(var) );
	trap_FS_Write( string, strlen( string ), configFile );

	trap_Cvar_VariableStringBuffer("inven_kevlar", var , sizeof(var ) );
	Com_sprintf( string, sizeof(string), "seta inven_kevlar %i \n", atoi(var) );
	trap_FS_Write( string, strlen( string ), configFile );

	// ammo

	trap_Cvar_VariableStringBuffer("inven_ammo_flash", var , sizeof(var ) );
	Com_sprintf( string, sizeof(string), "seta inven_ammo_flash %i \n", atoi(var) );
	trap_FS_Write( string, strlen( string ), configFile );
	
	trap_Cvar_VariableStringBuffer("inven_ammo_smoke", var , sizeof(var ) );
	Com_sprintf( string, sizeof(string), "seta inven_ammo_smoke %i \n", atoi(var) );
	trap_FS_Write( string, strlen( string ), configFile );

	trap_Cvar_VariableStringBuffer("inven_ammo_mk26", var , sizeof(var ) );
	Com_sprintf( string, sizeof(string), "seta inven_ammo_mk26 %i \n", atoi(var) );
	trap_FS_Write( string, strlen( string ), configFile );

	trap_Cvar_VariableStringBuffer("inven_ammo_40mmgren", var , sizeof(var ) );
	Com_sprintf( string, sizeof(string), "seta inven_ammo_40mmgren %i \n", atoi(var) );
	trap_FS_Write( string, strlen( string ), configFile );

	trap_Cvar_VariableStringBuffer("inven_ammo_primary", var , sizeof(var ) );
	Com_sprintf( string, sizeof(string), "seta inven_ammo_primary %i \n", atoi(var) );
	trap_FS_Write( string, strlen( string ), configFile );

	trap_Cvar_VariableStringBuffer("inven_ammo_secondary", var , sizeof(var ) );
	Com_sprintf( string, sizeof(string), "seta inven_ammo_secondary %i \n", atoi(var) );
	trap_FS_Write( string, strlen( string ), configFile );

	// weaponlist

	trap_Cvar_VariableStringBuffer("inven_primary", var , sizeof(var ) );
	Com_sprintf( string, sizeof(string), "seta inven_primary %i \n", atoi(var) );
	trap_FS_Write( string, strlen( string ), configFile );

	trap_Cvar_VariableStringBuffer("inven_secondary", var , sizeof(var ) );
	Com_sprintf( string, sizeof(string), "seta inven_secondary %i \n", atoi(var) );
	trap_FS_Write( string, strlen( string ), configFile );

  
	if ( !Q_stricmp( filename, "tangos.cfg" ) )
	{
		trap_Cvar_VariableStringBuffer("ui_t_e_eyes", var , sizeof(var ) );
		Com_sprintf( string, sizeof(string), "seta ui_t_e_eyes \"%s\" \n",  var);
		trap_FS_Write( string, strlen( string ), configFile );

		trap_Cvar_VariableStringBuffer("ui_t_e_head", var , sizeof(var ) );
		Com_sprintf( string, sizeof(string), "seta ui_t_e_head \"%s\" \n",   var);
		trap_FS_Write( string, strlen( string ), configFile );

		trap_Cvar_VariableStringBuffer("ui_t_e_mouth", var , sizeof(var ) );
		Com_sprintf( string, sizeof(string), "seta ui_t_e_mouth \"%s\" \n",   var);
		trap_FS_Write( string, strlen( string ), configFile );

		trap_Cvar_VariableStringBuffer("ui_t_model", var , sizeof(var ) );
		Com_sprintf( string, sizeof(string), "seta ui_t_model \"%s\" \n", var );
		trap_FS_Write( string, strlen( string ), configFile );

		trap_Cvar_VariableStringBuffer("ui_t_skin", var , sizeof(var ) );
		Com_sprintf( string, sizeof(string), "seta ui_t_skin \"%s\" \n", var );
		trap_FS_Write( string, strlen( string ), configFile );  
	}
	else if ( !Q_stricmp( filename, "seals.cfg" ) )
	{
		trap_Cvar_VariableStringBuffer("ui_s_e_eyes", var , sizeof(var ) );
		Com_sprintf( string, sizeof(string), "seta ui_s_e_eyes \"%s\" \n", var );
		trap_FS_Write( string, strlen( string ), configFile );

		trap_Cvar_VariableStringBuffer("ui_s_e_head", var , sizeof(var ) );
		Com_sprintf( string, sizeof(string), "seta ui_s_e_head \"%s\" \n", var );
		trap_FS_Write( string, strlen( string ), configFile );

		trap_Cvar_VariableStringBuffer("ui_s_e_mouth", var , sizeof(var ) );
		Com_sprintf( string, sizeof(string), "seta ui_s_e_mouth \"%s\" \n", var );
		trap_FS_Write( string, strlen( string ), configFile ); 

		trap_Cvar_VariableStringBuffer("ui_s_model", var , sizeof(var ) );
		Com_sprintf( string, sizeof(string), "seta ui_s_model \"%s\" \n", var );
		trap_FS_Write( string, strlen( string ), configFile );

		trap_Cvar_VariableStringBuffer("ui_s_skin", var , sizeof(var ) );
		Com_sprintf( string, sizeof(string), "seta ui_s_skin \"%s\" \n", var );
		trap_FS_Write( string, strlen( string ), configFile ); 
	}
	
	trap_FS_FCloseFile( configFile );
}
 

void Script_WriteConfig(itemDef_t *item, char **args) {
	const char *val;
	if (String_Parse(args, &val)) {
		UI_WriteWeaponConfig(val);
	}
}
...
commandDef_t commandList[] =
{
  ...
  {"writeteamconfig", &Script_WriteConfig }
};
...
void Item_RunExtScript(itemDef_t *item, const char *s) {
  ...
  if (item && s && s[0]) {
    ...
    while (1) {
      ...
      // not in our auto list, pass to handler
       if (!bRan) {
        DC->setWeapon (&p, item->ownerParam );
      }
    }
  }
}
...
//
// contais right click fix by defcon-x/ t e a m - m i r a g e
//		(c) 2001 www.team-mirage.net defcon-x@team-mirage.net
//
	...
	if (multiPtr) {
	  if (Rect_ContainsPoint(&item->window.rect, DC->cursorx, DC->cursory) && item->window.flags & WINDOW_HASFOCUS && item->cvar) 
	  {
			if (key == K_MOUSE1 || key == K_ENTER || key == K_MOUSE2 || key == K_MOUSE3) 
			{
				...
				if ( key == K_MOUSE2 )
				{
					current -= 2; // -1 would be the actual current
				}

				if ( current < 0 && key == K_MOUSE2 )
				{
					current = max-1;
				}
...
static bind_t g_bindings[] = 
{
	...
	{"+moveleft", 	 'a',					-1,		-1, -1},
	{"+moveright", 	 'd',					-1,		-1, -1},
	{"+moveup",			 K_MOUSE2,			-1,		-1, -1},
	{"+movedown",		 K_MOUSE3,					-1,		-1, -1},
...
//	{"+button3", 		 K_MOUSE3,			-1,		-1, -1},
	...
	// radio commands; personal
	
	{"radio affirmative", -1,						-1,		-1, -1},
	{"radio negative", -1,						-1,		-1, -1},
	{"radio followme", -1,						-1,		-1, -1},
	{"radio coverme", -1,						-1,		-1, -1},
	{"radio takefire", -1,						-1,		-1, -1},
	{"radio espotted", -1,						-1,		-1, -1},
	{"radio reportingin", -1,						-1,		-1, -1},

// team commands
	{"radio gogo", -1,						-1,		-1, -1},
	{"radio regroup", -1,						-1,		-1, -1},
	{"radio check", -1,						-1,		-1, -1},
	{"radio takecover", -1,						-1,		-1, -1},
	{"radio retreat", -1,						-1,		-1, -1},
	{"radio reportin", -1,						-1,		-1, -1},
	{"radio guard", -1,						-1,		-1, -1},
	{"radio locknload", -1,						-1,		-1, -1},

// objective commands
	{"radio objstatus", -1,						-1,		-1, -1},
	{"radio objsafe", -1,						-1,		-1, -1},
	{"radio objlost", -1,						-1,		-1, -1},
	{"radio objdanger", -1,						-1,		-1, -1},
	{"radio objhome", -1,						-1,		-1, -1},
	{"radio bgot", -1,						-1,		-1, -1},
	{"radio bplaced", -1,						-1,		-1, -1},
	{"radio bdisarm", -1,						-1,		-1, -1}
...
void Item_Slider_Paint(itemDef_t *item) {
	...
	if (item->text) {
		...
		if ( item->textRect.novscreen )
			novscreen = qtrue;
	} else {
		x = item->window.rect.x;

		if ( item->window.rect.novscreen )
			novscreen = qtrue;
	}
	DC->setColor(newColor);

	if ( novscreen )
		DC->drawStretchPic( x, y, SLIDER_WIDTH, SLIDER_HEIGHT, 0, 0, 1, 1, DC->Assets.sliderBar );
	else
	...
	if ( x > 640 )
		DC->drawStretchPic(  x - (SLIDER_THUMB_WIDTH / 2), y - 2, SLIDER_THUMB_WIDTH, SLIDER_THUMB_HEIGHT, 0, 0, 1, 1, DC->Assets.sliderThumb );
	else
...
void Item_Image_Paint(itemDef_t *item) {
	...
	if ( item->window.rect.novscreen )
		DC->drawStretchPic( item->window.rect.x+1, item->window.rect.y+1, item->window.rect.w-2, item->window.rect.h-2, 0, 0, 1, 1, item->asset );
	else
...
void Item_OwnerDraw_Paint(itemDef_t *item) {
  ...
  parent = (menuDef_t*)item->parent;
	...
	if (DC->ownerDrawItem) 
	{
		...
		if ( item->ownerParam )
		{
			DC->ownerDrawItem(item->window.rect.x, item->window.rect.y, item->window.rect.w, item->window.rect.h, item->textalignx, item->textaligny, item->window.ownerDraw, item->window.ownerDrawFlags, item->alignment, item->ownerParam, item->textscale, color, item->window.background, item->textStyle );
		}
		else if (item->text) 
		{
			Item_Text_Paint(item);
...
extern keywordHash_t menuParseKeywords[];

keywordHash_t *menuParse_Find(  char *keyword)
{ 
	int i;
	
	for (i = 0; menuParseKeywords[i].keyword; i++) {
		if (!Q_stricmp( menuParseKeywords[i].keyword , keyword ) )
			return &menuParseKeywords[i];
	}
	return NULL;/*
...
keywordHash_t *itemParse_Find(  char *keyword)
{ 
	int i;
	
	for (i = 0; itemParseKeywords[i].keyword; i++) {
		if (!Q_stricmp( itemParseKeywords[i].keyword , keyword ) )
			return &itemParseKeywords[i];
	}
	return NULL;/*
...
// decoration
qboolean ItemParse_teamcolor( itemDef_t *item, int handle ) {
	item->window.flags |= WINDOW_TEAMCOLOR;
	return qtrue;
}

qboolean ItemParse_strechpic( itemDef_t *item, int handle ) {
	item->useStrechPic = qtrue;
	return qtrue;
}
...
keywordHash_t itemParseKeywords[] = {
	...
	{"ownerParam", ItemParse_ownerParam, NULL },
	{"teamcolor", ItemParse_teamcolor, NULL },
	{"useStrechPic", ItemParse_strechpic, NULL },
	{"", NULL, NULL}
};
...
qboolean Item_Parse(int handle, itemDef_t *item) {
	...
	while ( 1 ) {
		...
		key = itemParse_Find( token.string );
		if (!key) {
...
//	key = KeywordHash_Find(menuParseKeywordHash, token.string);
		key = menuParse_Find( token.string );
		if (!key) {
