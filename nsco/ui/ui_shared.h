...
#define WINDOW_TEAMCOLOR		0x01000000	// switch color based on team( only for h.u.d. )
...
typedef struct itemDef_s {
  ...
  int cursorPos;                 // cursor position in characters
	...
	int ownerParam; // defcon-x. this is the parameter for items that are written into the memory as cvars (good, because we can also use config files)
	qboolean useStrechPic;
} itemDef_t;
...
// new
  void (*setWeapon)(char **p, int parameter );
